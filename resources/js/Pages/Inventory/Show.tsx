      ...
      {editing ? (
        <form onSubmit={handleUpdate} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              />
              {errors.productName && <p className="text-red-500 text-sm">{errors.productName[0]}</p>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full border p-2 rounded"
              />
              {errors.productCode && <p className="text-red-500 text-sm">{errors.productCode[0]}</p>}
            </div>
            ...
            {/* Selling Price */}
            <div>
              <label className="block text-sm font-medium">Selling Price (Auto)</label>
              <input
                type="number"
                name="sellingPrice"
            ...
            <div className="col-span-2">
              ...
              <input type="file" name="productImage" onChange={handleFileChange} className="w-full border p-2 rounded" />
            </div>
          </div>

          <div className="flex justify-end gap-2 pt-3">
            <button type="button" onClick={() => setEditing(false)} className="px-4 py-2 border rounded">
              Cancel
            </button>
        ...
        <div>
          ...
          <p>
            ...
            {seriasList.find((s) => s.id === product.seriasId)?.seriasNo || "-"}
          </p>
          <p>
            <strong>Description:</strong> {product.productDescription || "-"}
          </p>
          {product.productImage && (
            <img src={`/${product.productImage}`} alt="Product" className="mt-2 w-32 h-32 object-cover" />
          )}

          <div className="mt-3 flex gap-2">
            <button onClick={() => setEditing(true)} className="px-4 py-2 bg-yellow-500 text-white rounded">
              Edit
            </button>
          </div>
        </div>
